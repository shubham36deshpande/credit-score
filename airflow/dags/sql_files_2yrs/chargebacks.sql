SELECT 
        ...
        WHERE `bm`.`archived` = 0 and (cc.id is not null) and (`bm`.`transaction_date`>= DATE_SUB('{{ execution_date.strftime('%Y-%m-%d') }}', INTERVAL 729 DAY)) AND (`bm`.`transaction_date` <= '{{ execution_date.strftime('%Y-%m-%d') }}')
