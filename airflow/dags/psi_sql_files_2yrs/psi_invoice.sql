...
WHERE (crm_customers.quickbooks_id IS NOT NULL) 
    ...
    -- AND (PSI_Invoices.invoice_date >= DATE_SUB('{{ execution_date.strftime('%Y-%m-%d') }}', INTERVAL 729 DAY)) AND (PSI_Invoices.invoice_date <= '{{ execution_date.strftime('%Y-%m-%d') }}')
    AND (PSI_Invoices.invoice_date >= "2021-01-01") AND (PSI_Invoices.invoice_date <= '{{ execution_date.strftime('%Y-%m-%d') }}')
