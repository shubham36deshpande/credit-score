SELECT 
        ...
        WHERE `bm`.`archived` = 0 and (cc.id is not null) 
            -- and (`bm`.`transaction_date`>= DATE_SUB('{{ execution_date.strftime('%Y-%m-%d') }}', INTERVAL 729 DAY)) AND (`bm`.`transaction_date` <= '{{ execution_date.strftime('%Y-%m-%d') }}')
            and (`bm`.`transaction_date`>= "2021-01-01") AND (`bm`.`transaction_date` <= '{{ execution_date.strftime('%Y-%m-%d') }}')
